...
const VodiXSimple = artifacts.require('VodiXSimple');

contract('VodiXSimple', function ([owner, recipient, anotherAccount, ...otherAccounts]) {
  const initialSupply = new BN(1e11).muln(1e6).muln(1e6).muln(1e4);
  beforeEach(async function () {
    this.token = await VodiXSimple.new();
  });
  ...
  describe('ERC20Detailed', function () {
    ...
    beforeEach(async function () {
      this.detailedERC20 = await VodiXSimple.new();
    });
  ...
  describe('increase allowance', function () {
    ...
    describe('ERC20Burnable', function () {
      const initialBalance = new BN(1e11).muln(1e6).muln(1e6).muln(1e4);
    
      beforeEach(async function () {
        this.token = await VodiXSimple.new({ from: owner });
      });
